...
class VanillaSegLoss(nn.Module):
    ...
    def meta_logging(self, epoch, batch_id, batch_len, writer, pbar=None, meta_train_loss=0, meta_test_loss=0, mmd_loss=0):
        """
        Print out  the loss function for current iteration.
        ...
        if pbar is None:
            print("[epoch %d][%d/%d], || Loss: %.4f || meta train Loss: %.4f"
                " || meta test Loss: %.4f || mmd Loss: %.4f" % (
                    epoch, batch_id + 1, batch_len,
                    meta_test_loss + meta_train_loss, meta_train_loss, meta_test_loss, mmd_loss))
        else:
            pbar.set_description("[epoch %d][%d/%d], || Loss: %.4f || meta train Loss: %.4f"
                  " || meta test Loss: %.4f || mmd Loss: %.4f" % (
                      epoch, batch_id + 1, batch_len,
                      meta_test_loss+meta_train_loss+mmd_loss, meta_train_loss, meta_test_loss, mmd_loss))
        ...
        # writer.add_scalar('Dynamic_loss', dynamic_loss.item(),
        #                   epoch*batch_len + batch_id)
        writer.add_scalar('Meta_train_loss', meta_train_loss, epoch*batch_len + batch_id)
        writer.add_scalar('Meta_test_loss', meta_test_loss, epoch*batch_len + batch_id)
        writer.add_scalar('MMD loss', mmd_loss, epoch*batch_len + batch_id)
