...
class BaseDataset(Dataset):
    ...
    def reinitialize(self):
        ...
        training.
        """
        # Structure: {scenario_id : {cav_1 : {timestamp1 : {yaml: path, lidar: path, cameras:list of path, file_extension: list of path}}}}
        self.scenario_database = OrderedDict()
        self.len_record = []

        # loop over all scenarios
        for (i, scenario_folder) in enumerate(self.scenario_folders):
            self.scenario_database.update({i: OrderedDict()}) # self.scenerio_database: all scenario
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        data:
            cav_id:
                ego:
                time_delay:
                camera_params:  camera transformation matrix
                params: lidar params
                camera_np: 
                    camera01: np
                    camera02: np
                ...
                bev_static.png:          np
                bev_lane.png:            np
                bev_visibility.png:      np
                bev_visibility_corp.png: np
            car_id:
        ...
        # retrieve the corresponding timestamp key, e.g. 2 --> '000078'fang
        ...
        data = OrderedDict() # 一个scenario下所有的数据？
        
        # load files for all CAVs
        for cav_id, cav_content in scenario_database.items():   # scenario_database: {cav_1 : {timestamp1 : {yaml: path, lidar: path, cameras:list of path}}}
            ...
            # print(data[cav_id]['camera_np'])
            
            # random mixup amplitude
            camera_trans = OrderedDict()
            for camera_name, image_np in data[cav_id]['camera_np'].items():
                src2trg = self.transform_image(image_np)
                src2trg = cv2.cvtColor(src2trg, cv2.COLOR_RGB2BGR)
                camera_trans[camera_name] = src2trg
            
            data[cav_id]['camera_trans'] = camera_trans
            # print("data[cav_id]['camera_trans']: ", data[cav_id]['camera_trans'])
    ...
    def transform_image(self, src_img: np, trg_imgs_path = '/home/xhchen/hsk/CoBEVT/opv2v/DomainAlignment/demo_images'):
        
        src_img = cv2.cvtColor(src_img, cv2.COLOR_BGR2RGB)  # cv2.imread load the src_img in BGR, so first convert it to RGB 
        
        
        trg_img_list = os.listdir(trg_imgs_path)     # get the image list    

        trg_img_path = os.path.join(trg_imgs_path, random.choice(trg_img_list)) # random select a image and open it
        ...
        src2trg = self.domain_transfer_np(src_img, trg_img, L=0.002)
        src2trg = src2trg.transpose((1,2,0))
        src2trg = np.clip(src2trg, 0, 255).astype(np.uint8)
    ...
    def domain_transfer_np(self, src_img, trg_img, L=0.1 ):
        # exchange magnitude
    ...
    def retrieve_by_idx(self, idx):
        ...
        for i, ele in enumerate(self.len_record):
            if idx < ele:
                scenario_index = i
                break
        # scenario_database: one scenerio
    ...
    def reform_camera_param(self, cav_content, ego_content, timestamp):
        ...
        # load each camera's world coordinates, extrinsic (lidar to camera) pose and intrinsics (the same for all cameras).
