...
class CamIntermediateFusionDataset(base_camera_dataset.BaseCameraDataset):
    ...
    def __getitem__(self, idx):
        ...
        camera_trans_data = []  # shape is same as above
        ...
        for cav_id, selected_cav_base in data_sample.items():
            distance = common_utils.cav_distance_cal(selected_cav_base,
                                                     ego_lidar_pose)
            if distance > opencood.data_utils.datasets.COM_RANGE:  # 如果距离过大则过滤掉
                continue
            ...
            camera_trans_data.append(selected_cav_processed['camera']['data_trans'])
        ...
        camera_trans_data = np.stack(camera_trans_data)
        ...
        processed_data_dict['ego'].update({
            ...
            'camera_trans_data':camera_trans_data,
    ...
    @staticmethod
    def get_pairwise_transformation(base_data_dict, max_cav):  # 这应该是点云的
        """
    ...
    def get_single_cav(self, selected_cav_base):
        ...
        # update the transformation matrix (lidar)
        transformation_matrix = \
        ...
        all_camera_trans_data = []     # 幅度谱转换后的data
        all_camera_trans_origin = []
        ...
        # preprocess the input transformed rgb image
        for camera_id, camera_trans_data in selected_cav_base['camera_trans'].items():
            all_camera_trans_origin.append(camera_trans_data)
            camera_trans_data = self.pre_processor.preprocess(camera_trans_data)
            all_camera_trans_data.append(camera_trans_data)
        ...
        camera_dict = {
            ...
            'origin_data_trans': np.stack(all_camera_trans_origin),
            'data_trans': np.stack(all_camera_trans_data)
    ...
    def collate_batch(self, batch):
        ...
        for i in range(len(batch)):
            ...
            camera_trans_data = ego_dict['camera_trans_data']
            ...
            cam_rgb_all_batch_trans.append(camera_trans_data)
        ...
        cam_rgb_all_batch_trans = torch.from_numpy(
            np.concatenate(cam_rgb_all_batch_trans, axis=0)).unsqueeze(1).float()
        ...
        output_dict['ego'].update({
            ...
            'inputs_trans': cam_rgb_all_batch_trans,
