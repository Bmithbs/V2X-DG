...
class CorpBEVT(nn.Module):
    ...
    def forward(self, batch_dict, if_trans=False):
        
        if not if_trans:
            x = batch_dict['inputs']
        else:
            x = batch_dict['inputs_trans']
            # print(x.shape)
        # print("batch_dict is:", batch_dict)
        # print(x.shape)  # torch.Size([2, 1, 4, 512, 512, 3])   CAVsNum x 1(?) x CameraNum x 512 x 512 x RGBchannel 
        b, l, m, _, _, _ = x.shape    # batchsize, l, m , 
